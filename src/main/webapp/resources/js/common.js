    ...
    var selector   = '[data-modules-customtoggle]',
        ...
        Customtoggle = Widget.extend({
            ...
            init: function (element, config){
                ...
                // });
                //tag list 너비가 contents 너비보다 작은 경우 더보기 생성 안함
                // 다른 컨텐츠에서 문제가 생겨 disable
...
/**
 * @name vodPreview
 * @selector [data-modules-vodPreview]'
 */
;(function(core, $, undefined){
    "use strict";
    var win = $(window),
        ...
        Default = {
            delay: 2000,
            limit: null,    // 별도 지정값이 없을 경우 전체 재생됨
            timer: null
        },
        name = "vodPreview",
        namespace = ".vodPreview",
        ui = core.ui,
        Widget = ui.Widget,
        VodPreview = Widget.extend({
            name : name,
            init : function(element, config){
                var _ = this;
                var options = _.options = $.extend({}, Default, config);
            ...
            _bindEvents : function(){
                ...
                _.element
                    .on('mouseenter' + namespace, function(e){
                        _.options.timer = setTimeout(function() {
                            // 비디오 재생
                            _._playVideo();

                            // 비디오 진행 시간
                            _.element.find("video").on("timeupdate", function(){
                                _._timeUpdate();
                            });
                        }, _.options.delay);
                    })
                    .on('mouseleave' + namespace, function(e){
                        // 마우스 오버 시간 해제
                        clearTimeout(_.options.timer);

                        // 비디오 진행 시간 체크 해제
                        _.element.find("video").off("timeupdate");
            ...
            _timeUpdate : function(){
                ...
                if(_.options.limit != null && _.options.limit != 0){
                    // 5초 이상 재생 될 경우 0초부터 재시작
                    if(_.element.find("video")[0].currentTime >= 5){
                        _._pauseVideo();
                        _._playVideo();
                    }
                }
            ...
            _playVideo : function(){
                ...
                _.element.find("video")[0].play();
            },
            _pauseVideo : function(){
                var _ = this;

                // 비디오 멈춘 뒤 처음으로 돌림
                _.element.find("video")[0].pause();
                _.element.find("video")[0].currentTime = 0;
            }
        });
    ui.plugin(VodPreview);
})(window[LIB_NAME], jQuery);
...
;(function(core, $, undefined){
    ...
    var Default = {
        ...
        randomBackgroundSetForSlick: '[data-bgset-slick]',
    ...
    commonUi = Widget.extend({
		...
		initLibray : function(){
            ...
            if($(_.options.randomBackgroundSetForSlick).length > 0){
                _.randomBackgroundSetForSlick($(_.options.randomBackgroundSetForSlick));
            }
        ...
        randomBackgroundSetForSlick : function(el){
            var _ = this;
            var target = el;
            forEach.call(target, function (item, i) {
                var $this = $(item);
                var colors = $this.data('bgset-slick').split('|');
                var first = parseInt(Math.random() * colors.length);
                var childlen = $this.children().length;
                if (!$this.hasClass('_bgsetcomp')) {
                    $this.addClass('_bgsetcomp');
                    console.log(childlen)
                    for(i=0;i<childlen;i++){
                        $this.children().eq(i).css('background', colors[(first+i)%colors.length]);
                    }
                }
            });
        },
