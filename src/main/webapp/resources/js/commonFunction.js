...
//스마트폰에서 다음과 같은 파라미터로 넘어오면 m.hyundaihmall.com으로 안보낸다.
...
//이메일 수신거부 거절시페이지에서 접근시 m.hyundaihmall.com으로 안보낸다.
...
jQuery(function($){
    ...
    // live->on 함수로 변경 jquery1.9 deprecated
//     $(document).on('click',  targetLayerPopup2, function(){
//         var $this = $(this),
//             target = $(this.hash),
//             margin = 1,
//             // jquery version에 따른 error 발생으로 주석처리
...
function showBlockLoadingBar() {
	...
	loading += '        <p class="tit">결제가 진행중입니다…</p>';
...
$(function() {
    ...
    //임시적용 => jsp 단에서 commonUi('initLibrary') 완료 후 실행
    setTimeout(
        function(){
...
$(document).on('click', ".refer-popup .close-btn", function(){ //이 이벤트는 추후 삭제 필요.
...
/* 제휴사 팝업 닫기 이벤트 추가. 시점차이 문제로 반영 후 위에꺼 제거 하면됨.*/
