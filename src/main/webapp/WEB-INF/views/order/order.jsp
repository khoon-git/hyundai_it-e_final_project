								...
								<ul>
									<c:forEach items="${orderPrdList}" var="orderPrd">
										<li name="orderItem">
											...
											<input type="hidden" name="prd_board_id" value="${orderPrd.prd_board_id}">
											
											<a href="./p/${orderPrd.prd_board_id}" target="_blank">
											<span class="img">
...
function order() {
	...
	if ( $("li[name=orderItem]").length > 1) {
		val_name +=  + ' 외' + $("li[name=orderItem]").length;
	}
	...
	IMP.request_pay({
		...
		if (rsp.success) {
			...
			var address_dest = val_buyer_addr; //배송지
			...
			$("li[name=orderItem]").each( function(index) {			
				...
				var prd_board_id = $("input[name=prd_board_id]").eq(index).val();
