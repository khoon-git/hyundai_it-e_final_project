...
$(function() {
	$("input:radio[name='gender']:radio[value='${userTDTO.gender}']").prop('checked',true);
	$("#hp_no").val(checkHpReg('${userTDTO.hp_no}'));
});
</script>
</head>
	...
	<!-- <div id="contents" class="subPage"> -->
		<div id="sign">
			...
			<article class="sign_up">
				<!-- 정보입력 -->
				<form:form name="signUpUserForm" id="signUpUserForm" commandName="userTDTO" method="post" action="${app}/mypage/chngUsrInfo">
					<%-- <input type="hidden" id="token" data-token-name="${_csrf.headerName}" placeholder="Password" value="${_csrf.token}"> --%>
					<sec:csrfInput/>
					<input type="hidden" id="user_id" name="user_id" value="${userTDTO.user_id}" />
					<!-- <input type="hidden" name="check_agree0" value="N" /> 
					<input type="hidden" name="check_agree1" value="N" /> -->
					<input type="hidden" id="email" name="email" value="${userTDTO.email}" />
								...
								<div class="inner808">
									<div class="terms_area">
											...
											<div>
												...
												<div class="wrap_inp">
													...
													<div class="inp_bundle registerHpNo">
														<input type="text" name="hp_no" id="hp_no" class="inp flex" value=""/>
												...
												<div class="wrap_inp">
													...
													<div class="inp_bundle email_bundle emailCheckMsg">
														<input type="text" title="이메일 아이디 입력" id="registerEmail" name="registerEmail" value="${userTDTO.email}" maxlength="80" class="inp flex" placeholder="이메일">
												...
												<div class="wrap_inp">
													...
													<div class="inp_bundle registerUserNm">
														<input type="text" name="user_nm" id="user_nm" class="inp flex" value="${userTDTO.user_nm}" />
													</div>
												...
												<div class="wrap_inp">
													...
													<div class="radio_ui type1">
														<input type="radio" name="gender" id="gender_female" value="female"/> <label for="gender_female">여</label>
												...
												<div class="wrap_inp">
													...
													<div class="inp_bundle registerBirth">
														<input type="text" id="datepicker1" name="birth" class="inp flex" 
															value="<fmt:formatDate value="${userTDTO.birth}" pattern = "yyyy-MM-dd"/>" />
													</div> 
												...
												<div class="wrap_inp">
													...
													<div class="inp_bundle address_zip">
														<input type="text" name="address_f" id="address_f" class="inp flex" value="${userTDTO.address_f}" />
												...
												<div class="wrap_inp">
													...
													<div class="inp_bundle address_detail">
														<input type="text" name="address_l" id="address_l" class="inp flex" value="${userTDTO.address_l}" />
													</div>
										...
										<ul class="bullet_type1 mark2">
											<li>모든 항목을 입력하지 않으실 경우 회원 정보 수정이 불가능합니다.</li>
										</ul>
									</div>
	...
	<script type="text/javascript">
		document.title = "정보 입력 | 현대Hmall 정보수정";
		...
		$("#phoneChk").click(function(){
			...
			if (!checkHpNo()){ // 휴대폰 번호가 올바르지 않다면
				...
				$.ajax({
			        type:"GET",
			        url:"${app}/signUp/phoneCheck?phone=" + phone,
			        cache : false,
		...
		function submitUserInfo() {
			...
			if (validateJoinForm()){
				...
				$.ajax({
					type:"post",
					url:"${app}/mypage/chngUsrInfo",
					contentType:'application/json; charset=utf-8',
					data: JSON.stringify({
						'user_id' : $("#user_id").val(),
						'user_nm' : $("input[name=user_nm]").val(),
						'email' : $("#email").val(),
						'birth' : $("input[name=birth]").val(),
						'address_f' : $("#address_f").val(),
						'address_l' : $("#address_l").val(),
						'hp_no' : $("#hp_no").val(),
		...
		function validateJoinForm() {
			...
			if(bCheck && $("#email").length > 0 ) {
				...
				else
				{
					...
					$.ajax({
						type : 'POST',
						url : "${app}/mypage/checkEmailDupNotMeJSON",
						data: $("#email").val(),
						dataType: 'text',
						contentType:'application/json; charset=utf-8',
						async : false,
		...
		$("#datepicker1").blur(function() {
			...
			$("#datepicker1").parent().removeClass("focus");
		});
		...
		$("#address_f").blur(function() {
			...
			$("#address_f").parent().removeClass("focus");
		});
		...
		$("#address_l").blur(function() {
			...
			$("#address_l").parent().removeClass("focus");
		});
		...
		$("#registerEmail").blur(function(e) {
			$("#email").val( $("#registerEmail").val() + $("#registerWrite3_3 option:selected").text());
			checkEmailFormat();
			$("#registerEmail").parent().removeClass("focus");
		});
		// 이름 입력시 체크
		...
		function checkUserNm() {
			...
			$("#user_nm").parent().removeClass("focus");
		...
		function checkHpNo() {
			...
			$("#hp_no").parent().removeClass("focus");
		...
		function checkHpReg(hp) {
			return hp.replace(/[^0-9]/g, "").replace(/(^02|^0505|^1[0-9]{3}|^0[0-9]{2})([0-9]+)?([0-9]{4})$/,"$1-$2-$3").replace("--", "-");
		}
